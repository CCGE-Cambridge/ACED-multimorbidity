...
smoking_stat_clean <- function(df_smoke){
    df_smoke <- df_smoke %>%
        mutate(smoking_status= na_if(smoking_status, -3), 
            smoking_labels = recode(smoking_status, `0` = "never", `1` = "former", `2` = "current"))
    return(df_smoke)
}

##ethnicity
ethnicity_clean <- function(df_ethnicity){
    df_ethnicity <- df_ethnicity %>%
    mutate(ethnicity = case_when((ethnicity >= 1001 & ethnicity <= 1003) | ethnicity == 1 ~ 1, 
                                 (ethnicity >= 2001 & ethnicity <= 2004) | ethnicity == 2 ~ 2,
                                 (ethnicity >= 3001 & ethnicity <= 3004) | ethnicity == 2 ~ 3,
                                 (ethnicity >= 4001 & ethnicity <= 4003) | ethnicity == 4 ~ 4,
                                 ethnicity == 5 ~ 5, 
                                 ethnicity == 6 ~ 6,
                                 ethnicity == - 1 | ethnicity == -3 ~ NA_real_, 
                                 TRUE ~ NA_real_), 
            ethnicity_labels = recode(ethnicity, `1` = "white", `2` = "mixed", `3` = "se_asian", `4` = "black", `5` = "chinese", `6` = "other")) 

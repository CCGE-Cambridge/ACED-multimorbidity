...
gp_script_query_update <- function(df_gp_scripts,  df_codelist) {
    #takes gp_script and pre-formatted codelist
    #searches by data provider and recombines final list of events
    #formats issue date to event date
    #call function to censor by data provider 
    df_script_events_scot_bnf <- inner_join(df_gp_scripts[data_provider == 2], df_codelist[Type == "bnf_6_char"], by = c("bnf_code" = "Code"))
    df_script_events_scot_readv2 <- inner_join(df_gp_scripts[data_provider == 2], df_codelist[Type == "readv2"], by = c("read_2" = "Code"))
    df_script_events_scot <- distinct(rbind(df_script_events_scot_bnf, df_script_events_scot_readv2), .keep_all = TRUE)

    df_script_events_wales <- inner_join(df_gp_scripts[data_provider == 4], df_codelist[Type == "readv2"], by = c("read_2" = "Code"))
    
    df_script_events_engvis_dmd <- inner_join(df_gp_scripts[data_provider == 1], df_codelist[Type == "dmd"], by = c("dmd_code" = "Code"))
    df_script_events_engvis_readv2 <- inner_join(df_gp_scripts[data_provider == 1], df_codelist[Type == "readv2"], by = c("read_2" = "Code"))
    df_script_events_engvis <- distinct(rbind(df_script_events_engvis_dmd, df_script_events_engvis_readv2), .keep_all = TRUE)
